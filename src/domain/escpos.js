...
export function settlementEscPos(s){const asAr=(v)=>String(Math.round(Number(v)||0));const rows=['COD SETTLEMENT','-'.repeat(42),leftRight('DATE:',String(s?.date||'-')),leftRight('COURIER:',String(s?.courier||'-')),leftRight('PARCELS:',String(s?.parcelCount??(s?.parcels||[]).length)),'-'.repeat(42),leftRight('EXPECTED:',`${asAr(s?.expectedAr)} AR`),leftRight('COLLECTED:',`${asAr(s?.collectedAr)} AR`),leftRight('VARIANCE:',`${asAr(s?.varianceAr)} AR`),leftRight('METHOD:',String(s?.method||'cash'))];if(s?.note)rows.push(`NOTE: ${clip(String(s.note),36)}`);rows.push('-'.repeat(42),'SIGNATURE: ____________________','','');return concat(CMD.init,CMD.center,CMD.boldOn,line('LZWAY'),CMD.boldOff,CMD.left,encodeText(rows.join('\n')+'\n\n'),CMD.cut);}
