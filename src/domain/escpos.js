...
function compactQrData(value){return String(value||'').trim().replace(/#\/tracking$/i,'');}
...
export function labelEscPos(parcel,options={}){
 const created=parcel.createdAt?new Date(parcel.createdAt):new Date(); const stamp=localStamp(created); const qrData=options.trackingUrl||parcel.trackingUrl||parcel.trackingToken||parcel.pickId||'LabelOnZeWay'; const addressLines=wrap(parcel.customer?.address||'',42,3); const collect=Number(parcel.collect||0).toLocaleString('fr-FR').replace(/\u202f/g,' '); const delivery=Number(parcel.deliveryCharge||0).toLocaleString('fr-FR').replace(/\u202f/g,' '); const paymentMode=asciiText(parcel.paymentMode||'Especes'); const divider='------------------------------------------'; const parts=[CMD.init,CMD.left,CMD.normal];
 parts.push(CMD.boldOn,line('LABELONZEWAY'),CMD.boldOff,line(leftRight('Ship Smarter. Deliver Further.',stamp.time)),line(divider));
 ...
 parts.push(CMD.center,CMD.boldOn,line('Misaotra betsaka !'),CMD.boldOff,line('Merci pour votre confiance !'),line('LABELONZEWAY | PEOPLE. PARCELS. PROGRESS.'),line(''),CMD.cut); return concat(...parts);
}
export function manifestEscPos(parcels,title='LABELONZEWAY MANIFEST'){const rows=[title,'-'.repeat(42)];for(const parcel of parcels){rows.push(`${clip(parcel.pickId,12).padEnd(12)} ${clip(parcel.customer?.name,18).padEnd(18)} ${String(parcel.qty??'').padStart(3)}`);rows.push(`  ${clip(parcel.status,14)}  COLLECT ${String(parcel.collect??0)} AR`);}rows.push('-'.repeat(42),`${parcels.length} PARCEL${parcels.length===1?'':'S'}`);return concat(CMD.init,encodeText(rows.join('\n')+'\n\n'),CMD.cut);}
