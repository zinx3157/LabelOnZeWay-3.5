  ...
  return {
    render(state) {
      ...
      if (draft.step === 2) {
        ...
        const total = document.createElement('div'); total.className = 'calculation';
        const refresh = () => { total.textContent = `Collect: ${formatAr(calculateCollect(qty.input.value, price.input.value))} Ar`; };
        qty.input.addEventListener('input', refresh); price.input.addEventListener('input', refresh); refresh();
        const actions = document.createElement('div'); actions.className = 'button-row';
        const back = action('Back'); const next = action('Review label', 'primary');
        ...
        actions.append(back, next); panel.append(qty.wrap, price.wrap, total, notes.wrap, actions);
      }

      if (draft.step === 3) {
        const preview = document.createElement('article'); preview.className = 'label-preview';
        ...
        const actions = document.createElement('div'); actions.className = 'button-row';
        const back = action('Edit parcel'); const save = action('Save label', 'primary'); const print = action('Print test');
        back.addEventListener('click', () => store.update((current) => ({ ...current, labelDraft: { ...current.labelDraft, step: 2 } })));
        const materialize = (current) => ({ id: makeId('parcel'), pickId: makePickId(current.parcels), trackingToken: makeTrackingToken(), customerId: current.labelDraft.customerId, customer: current.labelDraft.customer, ...current.labelDraft.parcel, status: 'ready', createdAt: new Date().toISOString() });
        print.addEventListener('click', async () => {
          const parcel = materialize(store.getState());
          try { const result = await services.print.print({ data: bytesToBase64(labelEscPos(parcel)), labels: 1 }); print.textContent = result.adapter === 'cloud' ? 'Queued to cloud' : 'Printed'; }
          catch (error) { print.textContent = `Print failed`; store.setState({ ui: { ...store.getState().ui, notice: error.message } }); }
        });
        save.addEventListener('click', () => {
          store.update((current) => {
            let customerId = current.labelDraft.customerId; let customers = current.customers;
            if (!customerId) {
              ...
              if (match) customerId = match.id; else { customerId = makeId('customer'); customers = [...customers, { id: customerId, ...current.labelDraft.customer }]; }
            }
        ...
        actions.append(back, print, save); panel.append(preview, actions);
      }
